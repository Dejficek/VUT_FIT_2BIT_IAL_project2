...
**/
    ...
    //dokud nedojde na prázný strom pokračuje a přechází na podstromy podle velikosti proměnné content
    while(*tmp != NULL){
        if(Content < (*tmp)->Cont)
            tmp = &(*tmp)->LPtr;
        else if(Content > (*tmp)->Cont)
            tmp = &(*tmp)->RPtr;
        else            //když narazí na existující prvek se stejnou hodnotou, nic se neděje
            return;
    }

    //jakmile dojde na konec stromu alokuje novou paměť a nastaví hodnoty
...
**/
    ...
    //dokud nedojde na prázný kořen, pushne aktuální pointer a pokračuje do levého podstromu
	while(ptr != NULL){
	    SPushP(Stack, ptr);
	    BTWorkOut(ptr);
        ptr = ptr->LPtr;
